...
exports.reservarCadeiras = async (req, res) => {
  ...
  try {
    ...
    if (verificacao.length !== cadeiras_ids.length) {
      await conn.rollback();
      return res.status(400).json({ erro: 'Uma ou mais cadeiras já estão reservadas ou em uso.' });
    }
    ...
    await conn.rollback();
    res.status(500).json({ erro: 'Erro ao reservar as cadeiras', detalhes: error.message });
  } finally {
    conn.release();
  }
};
